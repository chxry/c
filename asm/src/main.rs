    ...
    match t {
      ...
      Token::Instruction(i) => match i {
        ...
        Instruction::Jmp => match eval_val(&mut tokens, &labels)? {
          ...
          Value::Const(to) => out.extend([OpCode::JmpC.to(), to]),
          Value::Deref(to) => out.extend([OpCode::JmpD.to(), to]),
        },
        Instruction::Db => {}
        Instruction::Add => {
          ...
          match eval_val(&mut tokens, &labels)? {
            ...
            Value::Const(src) => out.extend([OpCode::AddC.to(), dest.to(), src]),
            Value::Deref(src) => out.extend([OpCode::AddD.to(), dest.to(), src]),
          }
        }
      },
      Token::Eof => {}
...
impl<'a> Token<'a> {
  ...
  fn const_from_radix(s: &str, radix: u32) -> Result<Self> {
    Ok(Self::Const(u8::from_str_radix(&s[2..], radix)?))
  }
}
